...
static RET_LABEL: &'static str = "label_return";
static RET_REG_ADDR: &'static str = "ra_return";
...
impl CodeGenerator {
    ...
    pub fn add_func_begin(&mut self,
        ...
        if ret_type != CGType::TVoid {
            self.add_return_alloca(ret_type);
        }

        let mut arg_addrs: Vec<AddrRegister> = Vec::new();
        for arg in &arg_regs {
            let addr_reg = self.add_alloca(arg.0);
            self.add_store(&addr_reg, &RegOrInt::Reg(arg.1.clone()));
    ...
    pub fn add_func_end(&mut self, ret_type: CGType) {
        self.add_return_jump();
        self.add_return_label();

        if ret_type == CGType::TVoid {
            self.add_line(format!("ret void"));
        } else {
            let reg = self.add_return_load(ret_type);
            self.add_line(format!("ret {} {}", ret_type, reg));
        }
    ...
    // memory
    pub fn add_string_constant(&mut self, s: &String) -> Register {
        self.last_str_const += 1;
        let reg = Register(format!(".str_const_{}", self.last_str_const));
    ...
    pub fn add_str_load(&mut self, str_size: usize, const_reg: &Register) -> RegOrInt {
    ...
    pub fn add_return_alloca(&mut self, t: CGType) {
        let addr_reg = AddrRegister::new(t, String::from(RET_REG_ADDR));
        self.add_line(format!("%{} = alloca {}", &addr_reg, t));
    }
    ...
    pub fn add_return_load(&mut self, t: CGType) -> RegOrInt {
        let addr_reg = AddrRegister::new(t, String::from(RET_REG_ADDR));
        self.add_load(&addr_reg)
    }
    ...
    pub fn add_return_store(&mut self, t: CGType, val: &RegOrInt) {
        let addr_reg = AddrRegister::new(t, String::from(RET_REG_ADDR));
        self.add_store(&addr_reg, val);
    }
    ...
    pub fn next_reg(&mut self) -> Register {
        self.last_reg += 1;
        Register(format!("rv_{}", self.last_reg))
    }

    pub fn next_addr_reg(&mut self, t: CGType) -> AddrRegister {
        self.last_reg += 1;
        AddrRegister::new(t, format!("ra_{}", self.last_reg))
    }

    pub fn dummy_reg(&self) -> Register {
        Register(format!("r_dummy"))
    }

    // labels & brs
    pub fn next_label(&mut self) -> Label {
        self.last_label += 1;
        Label(format!("label_{}", self.last_label))
    }

    pub fn add_return_label(&mut self) {
        self.add_label(&Label(String::from(RET_LABEL)));
    }
    ...
    pub fn add_return_jump(&mut self) {
        self.add_jump(&Label(String::from(RET_LABEL)));
    }
...
impl AddrRegister {
    pub fn new(t: CGType, id: String) -> AddrRegister {
